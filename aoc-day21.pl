...
for  (@instructions) {
    $initial[$2] = $initial[$1] if /move position (\d+) to position (\d+)/;
    ($initial[$1], $initial[$2]) = ($initial[$2], $initial[$1]) if /swap position (\d+) with position (\d+)/;
    @initial[$1 .. $2] = reverse @initial[$1..$2] if /reverse positions (\d+) through (\d+)/;
    rotate $1, $2 if /rotate (left|right) (\d+) steps/;
    rotate('right', steps $1) if /rotate based on position of letter (\w)/;
}


print @initial, "\n";

#figure out how many steps to rotate
sub steps {
    my $char = shift;
    my ($steps) =  grep {$initial[$_] eq $char} 0 .. scalar @initial - 1;
    $steps ++;
    $steps ++ if $steps >= 4;
}
