...
for (@instructions) {
    my $c = 0;
    /^(bot \d+) gives low to (\w+ \d+) and high to (\w+ \d+)$/ && ++$c && rule $1, $2, $3;
    /^value (\d+) goes to (bot \d+)$/ && ++$c && value_to $1, $2;
    die unless $c;
}

# find the bots that have 2 chips
my @queue = grep {scalar @{$_->{chips}} == 2} values %bots;
...
while (1) {
    ...
    my $high = get_bot $bot->{high};
    push @{$high->{chips}}, $chips[1];
...
sub has_61_and_17 {
    my $bot = shift;
    my $chips = $bot->{chips};
    return scalar grep {$_ == 61 || $_ == 17} @$chips;
}
