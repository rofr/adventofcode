...
my $ssl = 0;
for my $line (@data) {
    my $bits = join '', map { abba } parse $line;
    $tls++ if $bits =~ /^.(0.)+$/  && $bits =~ /1/;
    my @aba = map { aba $_ } parse $line;
    my @odd = map { @$_ } @aba[ grep {$_ % 2} 0..$#aba ];
    my @even = map{ @$_ } @aba[ grep {$_ % 2 == 0} 0..$#aba ];
    $ssl++ if find_ssl \@odd, \@even;
}

print "num adresses that support tls: $tls\n";
print "num adresses that support ssl: $ssl\n";
...
sub aba {
    my $s = shift;
    my @matches = ();
    for my $i(0 .. -3 + length $s) {
        push @matches, $& if substr ($s, $i, 3) =~ /(.)(.)(\1)/ && $1 ne $2; 
    }
    return \@matches;
}
...
sub find_ssl {
    ...
    for  my $triplet ( @$a) {
        $triplet =~ /(.)(.)./;
        my $flipped = "$2$1$2";
        return 1 if exists $lookup{$flipped};
    }
    return 0;
}
