...
for my $line (@data) {
    ...
    if (real $room) {
        $sum += $room->{id};
        push @real_rooms, $room;
        $room->{decrypted} = decrypt $room;
        print "Decrypted: ->" , $room->{decrypted}, "\n";
        $answer = $room->{id} if $room->{decrypted} =~ /northpole object storage/;
    }
}

print "num real rooms ", scalar @real_rooms, "\n";
print "sum of valid rooms' sector ids: $sum\n";
print "id of room where northpole objects are stored: $answer\n";


sub decrypt {
    my $room = shift;
    my $result = $room->{name};
    $result =~ s/([a-z])/ rotate $1, $room->{id}/ge;
    $result =~ s/-/ /g; #dashes to spaces
    return $result;
}

sub rotate {
    my ($ch, $steps) = @_;
    my $offset = ord ($ch) - ord 'a';
    my $char_set_size = ord('z') - ord('a') + 1;
    return chr (($offset + $steps) % $char_set_size + ord 'a');
}


sub real {
    my $room = shift;
    my @chars = split '', $room->{name};
    my %counts = ();
    for my $ch(@chars) {
        $counts{$ch} ++ unless $ch eq '-';
    }
    my @sorted = sort {$counts{$b} <=> $counts{$a} || $a cmp $b} keys %counts;
    my $checksum = join '', @sorted[0..4]; 
    return $checksum eq $room->{checksum};
}

sub parse {
    my $line = shift;
    $line =~ /^(.+)-(\d+)\[(.+)\]$/;
    my $room = {name => $1, id => $2, checksum => $3};
    return $room;
}
